...
module.exports.requestService = async function (req, res) {
  try {
    let reciver_community = await db.public.user_community.findOne({
...
module.exports.acceptService = async function (req, res) {
  try {
    ...
    let provider_community = await db.public.user_community.findOne({
      where: { login_id: provider_id },
    });
    if (!provider_community) throw Error("Provider community not found");
    let provider_service = await db.public.provider_service.findOne({
      where: { login_id: provider_id },
    });
...
module.exports.updateStatus = async function (req, res) {
  try {
    ...
    let status_update = await db.public.request.update(
      { status: status },
      { where: { id: id } }
    );
    res.status(200).json({
...
module.exports.servicesAvailabeNow = async function (req, res) {
  try {
    ...
    let provider_service = await db.public.provider_service.findAll({
      where: { login_id: provider_id },
      attributes: ["service_id"],
    });
    let a = provider_service.map(({ service_id }) => parseInt(service_id));
    let available = await db.public.request.findAll({
      include: [
        {
          model: db.public.login,
          as: "reciever",
          attributes: ["id", "name", "profile_pic", "address"],
        },
        {
          model: db.public.services,
          attributes: ["id", "name", "description", "questions", "priceType"],
        },
      ],
      where: {
        reciver_community: provider_community.community_id,
        provider_id: null,
        service_id: a,
      },
      attributes: [
